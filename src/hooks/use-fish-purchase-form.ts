...
import {
  fishPurchaseFormSchema,
  type FishPurchaseFormData,
} from '@/lib/validation/fish-purchase';
...
export function useFishPurchaseForm(options: UseFishPurchaseFormOptions = {}) {
  ...
  const batchSetValue = useCallback(
    async (
      updates: Partial<FishPurchaseFormData>,
      ...
      if (options?.shouldValidate) {
        await trigger(
          Object.keys(updates) as Array<keyof FishPurchaseFormData>
        );
      }
    },
    [setValue, trigger]
    ...
    if (initialLocations && initialLocations.length > 0) {
      setState((prev) => {
        ...
        if (
          ...
          prev.locations.some(
            (loc, idx) => loc.id !== initialLocations[idx]?.id
          )
        ) {
  ...
  const handleNext = useCallback(() => {
    setState((prev) => {
      const steps: FishPurchaseFormStep[] = [
        'supplier',
        'details',
        'items',
        'review',
  ...
  const transformFormData = useCallback(
    (data: FishPurchaseFormData) => {
      return {
        ...data,
        ...(vehicleBookingId && {
          vehicle_booking_id: parseInt(vehicleBookingId),
        }),
