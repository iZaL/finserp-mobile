...
export function PdfReportGenerator({
  ...
  const handleDownloadPdf = async () => {
    ...
    setDownloading('pdf');
    try {
      const blob = await vehicleBookingService.exportReportPdf(
        dateRange.from,
        dateRange.to
      );
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      const blob = await vehicleBookingService.exportReportExcel(
        dateRange.from,
        dateRange.to
      );
  ...
  const shareReport = async () => {
    ...
    const reportData = {
      ...
      text:
        `Vehicle Booking Performance Report\n\n` +
    ...
    <Card className="p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold">{t('generateReport')}</h3>
